    ...
    >
      ...
      <div className="relative h-[220px] overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          <Badge className={p.tag === "FOR RENT" ? "bg-[#1a5c5c] text-white" : "bg-[#1A3C5E] text-white"}>
            {p.tag}
          </Badge>
        ...
        <div className="absolute bottom-3 left-3">
          <span className="text-white font-extrabold text-lg leading-none drop-shadow-lg">
            {p.price}
            {p.priceType && <span className="text-sm font-normal text-white/70">{p.priceType}</span>}
          </span>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="border-t border-gray-100 pt-3 flex items-center justify-between text-gray-500 text-xs">
          ...
          <span className="flex items-center gap-1.5"><BiArea size={13} /> {p.sqft} sqft</span>
          <motion.button
